    ...
    if use_goi:
        ...
        if species == 'mouse':
            ...
            functional_classes = {
                ...
                "CellChat Ligand": "CellChat_mouse_ligand.txt",
...
if 'deseq2' in st.session_state:
    ...
    if use_upload == "No":
        ...
        if  "deseq2_uploaded_file_name" in st.session_state:
            ...
            file_name_head = "res"
        input_file_type = 'tsv'
        if "Row_name" in df.columns.to_list(): # When containing Row_name
            df = df.set_index('Row_name')
            df.index.name = "Gene"

if "inv" not in st.session_state:
    st.session_state.inv = False # Inverse switch: True while inverted
...
# Handle cases where p-value is 0
    zero_adj = "No"
    ...
    if zero_adj == "Yes":
        if (P_column is not None) & (FC_column is not None):
            # Using max would result in inf, so first convert to nan, then use np.nanmax
   ...
   # if not enhanced:
    ...
    if not enhanced:
        with st.sidebar:
            ...
            Font_italic = False  # Default is non-italic
            ...
            if Mod_label:
                ...
                Font_italic = st.checkbox('Italic font?', value=False)  # Add italic option
     ...
     #       Font_family = "Arial Italic, Times New Roman Italic" if Font_italic else "Arial, Times New Roman"
         ...
         #   df['Color2'] = blue_rgb
            ...
            # Sort data order according to color
            df2 = df.copy()
 ...
 #   text_label[(df[P_column] >= L_P_threshold) | (np.abs(df[FC_column]) <= L_FC_threshold)] = None
    ...
    with st.sidebar:
        ...
        if Sp_genes:
            ...
            # Modified section for creating specified gene list
            gene_list_sp = []
            if len(genes) > 0:
                ...
                gene_list_all = [x for x in text_label if x is not None]  # Define gene_list_all here
                df['Point_Size'] = point_sizes

                text_label_color = text_label.copy() # For coloring
                ...
                if Sp_attribute:
                    ...
                    Sp_sig_alpha = 1.0  # Fix transparency for significant genes to 1.0 (fully opaque)

                    # RGBA color for non-significant specified genes
                    Sp_rgb_ns = hex2rgb(Sp_c, Sp_alpha)
                    # RGBA color for significant specified genes
                    Sp_rgb_sig = hex2rgb(Sp_c, Sp_sig_alpha)

                    for i in gene_list_color:
                        # Select appropriate transparency based on significance
        ...
        with st.sidebar:
            ...
            if Hl_gene_option:
                ...
                if len(Hl_genes) > 0:
                    ...
                    Hl_genes = Hl_genes.replace('"',"")
                    st.write(Hl_genes)
                    Hl_gene_list_sp = Hl_genes.split(' ') # First split by whitespace
                    Hl_gene_list_sp = list(filter(lambda a: a != '', Hl_gene_list_sp)) # Remove empty strings
                    if ',' in Hl_genes:
                        Hl_gene_list_sp = sum([x.split(',') for x in Hl_gene_list_sp],[]) # Flatten with sum: sum(x, [])
                    if '\t' in Hl_genes:
         ...
         #           text_label[-text_label.isin(gene_list)] = None
                    Hl_gene_set_lower = [x.lower() for x in set(Hl_gene_list_sp)] # Case-insensitive search
                    Hl_text_label_dat = []
    ...
    if st.button('Make plot'):
        if not enhanced:
            ...
            fig.update_layout(height=Y_size) # Set size
            ...
            # Font family setting (changed to Arial only) and adding style-italic
            if Font_italic:
            ...
            fig3.update_layout(height=Y_size) # Set size
