...
def extend_network_with_ppi(
  ...
  #  st.write(f"Number of nodes in network: {len(existing_nodes)}")
    ...
    # Step 2: Identify only transcription factors among the direct targets of the target gene
...
def cleanup_network_multi(network: nx.DiGraph, target_genes: List[str]) -> Tuple[nx.DiGraph, List[Tuple[str, str]]]:
    """
    Clean up network for multiple target genes.
    Path verification is performed independently for each target gene.

    Path conditions:
    1. Nodes on paths reaching any of the target genes (upstream)
    2. Nodes on paths reachable from any of the target genes (downstream)
    """
    cleaned_network = network.copy()
    nodes_to_keep = set(target_genes)  # Keep all target genes

    # 1. Check paths individually for each target gene
    for target_gene in target_genes:
        ...
        for node in network.nodes():
            if node != target_gene:
                try:
                    # Check if a path exists from node to target
        ...
        # Add all nodes reachable from target_gene
        try:
    ...
    # 2. Also add downstream nodes from nodes to keep
...
def create_interactive_multi_target_network(
    ...
    if hub_centric and hubs:
        ...
        # Place hub genes and target genes at the center
        hub_nodes = set(hubs) | set(target_genes)
        n_hubs = len(hub_nodes)

        # Arrange in a circle near the center
        ...
        # Place other nodes on the outside
...
def build_multi_target_network(
    ...
    """Build network for multiple target genes"""
    ...
    for target_gene in target_genes:
        ...
        for u, v, data in network.edges(data=True):
            if combined_network.has_edge(u, v):
                # Update weight to maximum value if edge already exists
        ...
        # Integrate regulators
        for level, tfs in regulators.items():
            if level != 'target_genes':  # target_genes is already set
...
def build_expanded_tf_network(
    ...
    """Build network for a single target gene (modified from existing function)"""
    tf_network = nx.DiGraph()
    regulators_by_level = {
...
def get_network_layout_multi(
    ...
    """Generate layout for multiple target genes

    Args:
        tf_network: Network graph
        target_genes: List of target genes
        layout_type: Layout type
        hub_centric: Whether to place hub nodes at the center
    ...
    Returns:
        Dict[str, np.ndarray]: Node position information
    """
    ...
    if hub_centric and (hubs or target_genes):
        ...
        # Place hub nodes and target genes at the center
...
def extend_multi_target_network_with_ppi(
    ...
    """Extend PPI network for multiple target genes"""
    extended_network = tf_network.copy()
    added_edges = []
    ppi_tfs = []
...
def visualize_multi_target_network(
 ...
 #  fig = plt.figure(figsize=(15, 12))
   width = height * (15/12)  # 16:9 aspect ratio
   fig = plt.figure(figsize=(width/96, height/96))  # Convert to inches for matplotlib
    ...
    #        st.write(f"{interaction[0]} - {interaction[1]}: {interaction[2]}")
        ...
        for protein1, protein2, score in all_interactions:
            ...
            if (protein1.upper() in normalized_proteins and
                ...
                interactions[orig_p2][orig_p1] = score  # Add bidirectional interactions
        ...
        if not interactions:
            ...
            # Display example interactions found
...
def convert_to_string_symbols(proteins: List[str], species: str = "9606") -> Dict[str, str]:
    ...
    for protein in proteins:
        # Basic cleaning (convert to uppercase as needed)
...
def filter_network_by_string(
    ...
    for source, target, data in original_edges:
        ...
        # Check interactions in STRING-db
        score = None
...
def load_trrust_data(species: str) -> Dict[str, List[Tuple[str, str, str]]]:
    ...
    Returns:
        ...
        and list of (target gene, interaction type, PMID) as value
    """
...
def filter_network_by_trrust(
    ...
    for source, target in list(tf_network.edges()):
        ...
        # Check both uppercase and lowercase for case differences
        ...
        if trrust_interactions:
            # Check if selected interaction types and minimum PMID count are satisfied
            valid_interactions = [
            ...
            if valid_interactions:
                ...
                # If number of PMIDs meets minimum requirement
...
def format_tf_name_for_chip_atlas(tf_name: str, species: str) -> str:
    ...
    """
    # For mouse, capitalize first letter and lowercase the rest
    if species == "mouse":
...
def get_chip_atlas_data(tf: str, species: str = "mouse", distance: int = 1000) -> pd.DataFrame:
    ...
    try:
        ...
        # If local file doesn't exist, get from API
...
def get_network_chip_data(network_tfs: List[str], species: str = "mouse",
    ...
    """Get ChIP-Atlas data for all TFs in the network"""
    chip_data = {}
...
def get_edge_chip_scores(tf_network: nx.DiGraph,
    ...
    """Get ChIP-Atlas score for each edge in the network"""
    edge_scores = {}
...
def load_tfs_list(file) -> Set[str]:
    """
    Load appropriate TF list based on uppercase/lowercase pattern of regulon data
...
def load_csi_data(file) -> Dict[str, Dict[str, float]]:
    ...
    for line in content[1:]:  # Skip header
        row = line.split(',')
        if len(row) >= 3:
            # Extract transcription factor names from columns 1 and 2 (remove digits onwards)
...
def get_connected_tfs(regulon_data: Dict[str, Set[str]],
    ...
    """Get all TFs connected to a TF and their CSI values"""
    connected_tfs = []
    ...
    # Also get downstream if in network_mode
    ...
    # Sort by CSI value and select top ones
    connected_tfs.sort(key=lambda x: x[1], reverse=True)
...
def calculate_weighted_centrality(tf_network: nx.DiGraph,
    ...
    try:
        # Set edge weight to 1.0 if not present in network
...
def get_edge_color(source_node, node_categories, color_scheme, target_genes, ppi_tfs=None, hubs=None):
    ...
    Args:
        ...
        ppi_tfs (list, optional): List of PPI transcription factors
        hubs (set, optional): Set of hub nodes
    """

    # Set to empty list if ppi_tfs is None
    ppi_tfs = ppi_tfs or []
    ...
    # Prioritize edges from target genes
    if source_node in target_genes:
        color_idx = f'target_{source_node}'
        return color_scheme.get(color_idx, color_scheme['target_genes'])  # Changed 'target' to 'target_genes'
    
    # Check hub node (excluding target genes)
    if hubs and source_node in hubs and source_node not in target_genes:
        return color_scheme['hub_node']
    
    # Check other node categories
...
def adjust_node_positions(pos: Dict[str, np.ndarray], min_dist: float = 0.2) -> Dict[str, np.ndarray]:
    ...
    for i, node1 in enumerate(nodes):
        for node2 in nodes[i+1:]:
            ...
            if dist < min_dist:
                # Repel nodes if distance is less than minimum
...
def save_static_network(tf_network: nx.DiGraph,
                       ...
                       layout_type: str = "spring",
                       layout_scale: float = 3.0,  # Added
                       min_node_dist: float = 0.2,  # Added
                       height: int = 800) -> None:
    
    # Same as previous code below
...
def add_save_buttons_to_main(st, filtered_network, regulators_by_level, target_genes, 
                             ...
                             layout_scale=3.0,  # Added
                             min_node_dist=0.2):  # Added
        ...
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix=f'.{static_format}') as tmp_file:
                save_static_network(
                    ...
                    layout_scale=layout_scale,  # Added
                    min_node_dist=min_node_dist,
                    height=viz_height
                )
            ...
            # Change filename to include all target genes
        ...
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix='.html') as tmp_file:
                save_interactive_network(
                    tf_network=filtered_network,
                    regulators_by_level=regulators_by_level,
                    target_genes=target_genes,  # Fixed: target_gene -> target_genes
                    output_path=tmp_file.name,
                    ppi_tfs=ppi_tfs,
                    font_size=font_size,
                    node_size_factor=node_size,
...
def get_hub_nodes(method: str, network: nx.DiGraph,
    ...
    Args:
        ...
        network: Network
        centrality_data: Centrality measure data
    ...
    if regulon_file and (csi_file or use_equal_weights):
        ...
        with st.form("network_settings"):     
            ...
            target_genes = st.multiselect(
                ...
                max_selections=5  # Up to 5 selections allowed
            )
