...
st.write("The count is the sum of raw counts of cells contained in each metacell.")
st.write("When using metacell data for analysis, treat it as count data and normalization is required.")
...
if uploaded_file is not None:
    ...
    with st.form("Basic settings:"):
        ...
        st.write("This is the number for the entire sample.")
...
#            n_cells = round(len(adata_dict[i].obs) / fracion_n_SEACs)
            ...
            try:
                ...
                st.write(f"Error: {e}")
                st.write('The nSEAcells for this sample is probably too small. Continuing without this sample.')
                del adata_dict[i]
    ...
    #    SEA2Cell_ad = SEACells.core.summarize_by_SEACell(adata_merged, SEACells_label='SEACell', celltype_label=cell_type)
        # Summarized data goes into SEA3Cell_ad.X sum
    ...
    #    ) # Copy adata.X to adata.raw.X
        ...
        def create_seacell_adata(adata_merged, seacell_key='SEACell'):
            ...
            # Create new X matrix (sum of cell counts)
            ...
            # Calculate average UMAP coordinates per SEACell
        ...
        # Use function to create new AnnData object
