...
st.markdown("""
This app adds metadata to an h5ad file from another h5ad file or TSV file.

### Usage
1. Upload **main h5ad file** (target for adding metadata)
2. Upload **metadata source** (h5ad file or TSV file)
3. Select **metadata columns** to add
4. Match by cell name (cell barcode) to add metadata
5. Download modified h5ad file (original_filename.mod.h5ad)
""")
...
main_h5ad = st.file_uploader(
    "Upload main h5ad file (target for metadata)",
    type=['h5ad'],
    key='main_h5ad',
    help="Upload h5ad file to add metadata to"
)
...
if main_h5ad is not None:
    ...
    metadata_source_type = st.radio(
        ...
        ["h5ad file", "TSV file"],
        help="Select metadata source type"
    )
    ...
    if metadata_source_type == "h5ad file":
        metadata_file = st.file_uploader(
            ...
            type=['h5ad'],
            key='metadata_h5ad',
            help="h5ad file to get metadata from"
        )
        ...
        metadata_file = st.file_uploader(
            ...
            type=['tsv', 'txt', 'csv'],
            key='metadata_tsv',
            help="TSV file containing metadata (1st column is cell barcode)"
        )
    ...
    if metadata_file is not None and 'metadata_df' in locals():
        ...
        with col1:
            ...
            remove_main_prefix = st.checkbox(
                ...
                value=False,
                help="e.g., 'Data1_AAACCCACAAGACCTT-1' → 'AAACCCACAAGACCTT-1'"
            )
            ...
            if remove_main_prefix:
                main_prefix_delimiter = st.text_input(
                    ...
                    value="_",
                    help="Character to separate prefix (e.g., '_')"
                )
                main_remove_parts = st.number_input(
                    ...
                    max_value=5,
                    value=1,
                    help="Number of leading parts to remove when split by delimiter"
                )

            remove_main_suffix = st.checkbox(
                "Remove suffix from main h5ad cell barcodes",
                value=False,
                help="e.g., 'AAACCCACAAGACCTT-1' → 'AAACCCACAAGACCTT'"
            )
            ...
            if remove_main_suffix:
                main_suffix_delimiter = st.text_input(
                    ...
                    value="-",
                    help="Character to separate suffix (e.g., '-')"
                )
        ...
        with col2:
            ...
            remove_meta_prefix = st.checkbox(
                ...
                value=False,
                help="e.g., 'E17_EC_1_AGATGAATCGAGTTGT-1' → 'AGATGAATCGAGTTGT-1'"
            )
            ...
            remove_meta_suffix = st.checkbox(
                ...
                value=False,
                help="e.g., 'AGATGAATCGAGTTGT-1' → 'AGATGAATCGAGTTGT'"
            )
        ...
        if len(common_cells) == 0:
            ...
            selected_columns = st.multiselect(
                ...
                help="Select metadata columns to add (multiple selection)"
            )
            ...
            if selected_columns:
                ...
                if conflicts:
                    ...
                    overwrite_mode = st.radio(
                        ...
                        help="How to handle duplicate column names"
                    )
                else:
                    overwrite_mode = None
