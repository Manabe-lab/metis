...
def cellchat_analysis(
   ...
   # debug_mode=False,
    ...
    try:
        ...
        # データの最大値で正規化（最適化）
        # data_use = normalize_data(X) Rではnormalizeデータを更に正規化はしていない
        data_use = X / np.max(X).astype(np.float64)  # Rの実装と同様に最大値で正規化
        ...
        data_use_avg_dict, cell_counts, valid_cell_types = calculate_mean_expression_optimized(
        ...
        all_gene_expr = precompute_gene_expressions(
            data_use, cell_labels, permutation, valid_cell_types, FunMean, nboot
        )
        ...
        for i in range(nLR):
            ...
            # 並列処理用関数
            def compute_permutation_batch(batch_indices):
            ...
            for b_start in range(0, nboot, batch_size):
                ...
                if n_jobs_to_use > 1 and len(batch_indices) > 1:
                    ...
                    batch_results_list = Parallel(n_jobs=n_jobs_to_use, backend="loky")(
                        delayed(compute_permutation_batch)([j]) for j in batch_indices
                    )
                    # 結果の結合
                    ...
                    # 単一スレッド処理
                    batch_results = compute_permutation_batch(batch_indices)
                    for j_idx, j in enumerate(batch_indices):
        ...
        # シグナル経路レベルでの通信確率を計算
        netP = computeCommunProbPathway({"prob": Prob, "pval": Pval}, resource, thresh=trim_threshold, apply_pval_filter=apply_pval_filter)
        ...
        # 集計ネットワークを計算
        net_summary = aggregateCell_Cell_Communication({"prob": Prob, "pval": Pval}, valid_cell_types, pval_threshold=0.05, apply_pval_filter=apply_pval_filter)
        progress_bar.empty()
