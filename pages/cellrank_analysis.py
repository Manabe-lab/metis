...
### Workflow
1. **Load file**: h5ad file from scVelo analysis results
...
if uploaded_h5ad is not None:
    ...
    with st.expander("📚 Parameter Guide", expanded=False):
        st.markdown("""
        ### Kernel
        - **VelocityKernel**: Transition probability based on RNA velocity (recommended)
        - **ConnectivityKernel**: Transition probability based on neighborhood graph
        - **Combined**: Combination of both (more robust)

        ### Terminal States
        - **Number of terminal states**:
          - **None (default)**: Auto-estimate from data (recommended)
          - **Numeric value**: Manual setting in range 1-20
          - Terminal states are the final fates of cell differentiation (e.g., mature cell types)

        - **Cluster key**:
          - Name of obs column containing cluster information (e.g., 'leiden', 'louvain', 'clusters')
          - Estimates terminal states using existing clustering results as reference
          - Enables more biologically meaningful state estimation by considering cluster structure
          - Automatically searches for 'leiden'/'louvain'/'seurat_clusters' if not found

        ### Fate Probability
        - Calculates the probability of each cell reaching each terminal state
        - Quantifies the cell fate decision process
    ...
    with st.form("cellrank_params_form"):
        ...
        kernel_type = st.selectbox(
            ...
            index=2,
            help="Method for calculating transition probability. Combined is recommended"
        )
        ...
        if kernel_type == "Combined (Velocity + Connectivity)":
            ...
            with col1:
                velocity_weight = st.slider(
                    ...
                    value=0.8,
                    step=0.1,
                    help="Weight for Velocity kernel (remaining weight goes to Connectivity kernel)"
                )
            with col2:
        ...
        with col2:
            n_states = st.number_input(
                ...
                max_value=20,
                value=None,
                help="Number of terminal states. Leave empty (None, default) for auto-estimation (recommended). Can also be manually set in range 1-20."
            )
        ...
        compute_initial_states = st.checkbox(
            ...
            value=False,
            help="Also estimate initial states (takes more time)"
        )
        ...
        with st.spinner("Loading and processing..."):
            ...
            try:
                ...
                if required_data:
                    ...
                    This file does not appear to be a scVelo analysis result.
    ...
    if st.session_state.cellrank_complete:
        ...
        - Initial states (`adata.obs['initial_states']`) (if computed)

        #### Recommended: Visualize with CellRank Visualization app

        First, visualize the results in the **CellRank Visualization** app:
        ...
        3. Available visualizations:
           ...
           - Fate probability heatmap (heatmap of all cells x all states)
           - Gene expression trends (gene expression along lineages)
           - Initial states (initial states, if computed)

        #### More detailed analysis: Pseudotime Gene Expression app

        You can analyze gene expression trends in detail using fate probability to each terminal state as the x-axis:
        1. Go to **Pseudotime gene expression** page
        2. Upload this h5ad file
        3. Select `fate_prob_{state}` in Pseudotime selection
        4. With x-axis = probability to that terminal state (0-1), you can:
           - Create heatmaps, line plots, stacked area plots
    ...
    st.info("👆 Upload an h5ad file from scVelo analysis results to begin")
