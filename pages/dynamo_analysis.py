...
### Workflow
1. **File Upload**: h5ad (optional) + loom (spliced/unspliced matrices)
2. **Preprocessing** (optional): Skip if using scVelo-processed h5ad (recommended)
3. **Dynamics Estimation**: RNA dynamics modeling (including moment calculations)
4. **Vector Field**: Reconstruction of continuous vector field
5. **Geometric Analysis**: Calculation of speed, curl, divergence, acceleration
6. **Save Results**: Download analysis results as h5ad file

### Dynamo Features
- **Vector field reconstruction**: Trajectory prediction using continuous velocity field
...
- **In silico perturbation**: Prediction of gene perturbation effects
- **Regulatory network**: Inference of transcriptional regulatory networks
...
2. **loom file**: **Filtered** loom file from Data filtering app
   - Contains spliced/unspliced matrices
   - Has same cell IDs as h5ad

#### ⚠️ When analyzing from loom file only

- Use **raw loom file not filtered by Data filtering app**
- Run Dynamo preprocessing (QC, normalization, feature selection, PCA)
- UMAP/cluster information will be calculated from scratch
""")
...
with col1:
    uploaded_h5ad = st.file_uploader(
        ...
        help="scVelo-processed h5ad file (with UMAP/clusters/QC)"
    )
...
with col2:
    uploaded_loom = st.file_uploader(
        ...
        help="Filtered loom file from Data filtering app (or raw loom)"
    )
...
if uploaded_loom is not None:
    if uploaded_h5ad is not None:
        st.success("✓ Both files uploaded - will use UMAP/cluster information from h5ad (recommended)")
        ...
        if len(available_embeddings) == 0:
            ...
            st.warning("⚠️ No embeddings found in h5ad. Using default umap/pca")
        else:
            st.info(f"✓ Available embeddings: {', '.join(available_embeddings)}")
    else:
        st.warning("⚠️ Loom file only - will run QC/normalization/UMAP calculation with Dynamo (use raw unfiltered loom)")
    ...
    if uploaded_h5ad is not None:
        ...
        **Basis selection for Vector Field calculation:**
        ...
        - **Can use**: PCA, scVI, Harmony (PCA-based)
          - These have loading matrices enabling reverse transformation to gene expression space
        - **Cannot use**: FastMNN (mnn)
          - FastMNN creates a new corrected space without loading matrices
          - Cannot interpret at gene level, so unusable for perturbation analysis
        - 💡 If performing perturbation analysis, be sure to calculate Vector Field using PCA or scVI basis
        """)
        ...
        st.info(f"""
        ✓ **Vector Field calculation targets (3):**
        ...
        3. **X_dynamo.umap** - New calculation (KNN consistent, created later)
        """)
    else:
        selected_basis = None
        selected_umap = None
        st.info("Loom file only: Vector Field will run on newly calculated UMAP")
    ...
    if skip_preprocess:
        with col2:
            recompute_hvg = st.checkbox(
                "Recompute HVGs",
                value=False,
                help="Use existing normalized data but recompute HVGs only"
            )
    else:
        recompute_hvg = False
