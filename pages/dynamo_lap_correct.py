...
if uploaded_h5ad is not None:
    ...
    with col1:
        ...
        if not vf_bases_for_lap:
            ...
            **LAP calculation requires high-dimensional space:**
            - ✅ PCA, MNN, etc.
            - ❌ UMAP, tSNE, etc. (low-dimensional embedding not allowed)

            Please calculate Vector Field with high-dimensional basis in Dynamo Analysis.
            """)
            st.stop()
        ...
        **Available high-dimensional bases for LAP:**
        ...
        lap_basis = st.selectbox(
            ...
            vf_bases_for_lap,
            help="Perform LAP calculation in high-dimensional space (PCA, MNN, etc.)"
        )
        ...
        if issues:
            ...
            for issue in issues:
                st.markdown(issue)
            st.info("""
            **Solution**: Please run the following in Dynamo Analysis app:
            1. Vector Field calculation
            2. Cell velocities calculation
    ...
    with col2:
        ...
        **Embedding for visualization:**
        ...
        vis_basis = st.selectbox(
            ...
            help="Embedding for result visualization (UMAP recommended)"
        )
    ...
    selected_cell_types = st.multiselect(
        ...
        help="LAP is calculated between all cell type pairs"
    )
    ...
    selection_method = st.radio(
        ...
        help="""
        - Centroid-nearest: Cell closest to centroid (recommended)
            ...
            try:
                ...
                for ct in selected_cell_types:
                    ...
                    if selection_method == "Centroid-nearest":
                        # Select cell closest to centroid (calculated in high-dimensional space)
                    ...
                    for i, start_ct in enumerate(selected_cell_types):
                        for j, end_ct in enumerate(selected_cell_types):
                            if start_ct != end_ct:
                                ...
                                try:
                                    # Step 1: LAP calculation on visualization basis (UMAP, etc.) - optional
                                    # This is only for saving visualization path
                                    ...
                                    # Step 2: LAP calculation in high-dimensional space (PCA, etc.) - required
                                    # This is the actual LAP calculation
                                    ...
                                    # Gene trajectory and ranking (calculated from high-dimensional LAP)
        ...
        **Computed LAP paths:**
        ...
        if len(transition_keys_list) == 0:
            ...
            with tab3:
                ...
                st.markdown("""
                Heatmap of gene expression dynamics along LAP path
                """)
                ...
                with st.form("heatmap_options_form"):
                    ...
                    custom_gene_input = st.text_area(
                        ...
                        help="Gene names separated by space/comma/tab/newline. Case insensitive comparison."
                    )
                    ...
                    with col_opt4:
                        show_gene_labels = st.checkbox(
                            "Show gene names",
                            value=True,
                            help="Show gene names on Y-axis"
                        )
                    ...
                    with col_size2:
                        fig_height = st.slider(
                            ...
                            value=10,
                            step=1,
                            help="Heatmap height (inches). Increase if many genes"
                        )
        ...
        st.markdown("""
        LAP analysis results can be exported as TSV files (tab-separated).

        **Files to save (bundled in ZIP):**
        ...
        with col_exp1:
            export_filename = st.text_input(
                ...
                help="Save TSV files bundled in ZIP"
            )
