...
def create_cell_color_mapping(cell_list, palette_name):
    """
    Function to create consistent mapping between cell names/clusters and colors
...
st.markdown("""
Visualize Dynamo analysis results in various ways.

### Visualization Types
- **Streamline plot**: Represent vector field flow as smooth curves
- **Cell-wise vectors**: Display vectors for each cell as arrows
- **Grid vectors**: Vector field on a grid
- **Phase portraits**: Spliced/unspliced dynamics for each gene
- **Topography**: Potential landscape (energy landscape) ⚠️ UMAP required
- **Geometric features panel**: Visualization of speed, divergence, acceleration, curvature
  - **Multiple basis support**: When Geometry is calculated for multiple bases, panels are automatically generated for all bases

### Multiple Basis Analysis Support
When Vector Field/Geometry is calculated for multiple bases (e.g., `rna.mnn.umap`, `rna.pca.umap`) in Dynamo Analysis:
- Vector Field visualization: Select each basis for individual visualization
- Geometric features panel: **Automatically generates panels for all bases** displayed side by side (for comparison)
...
if uploaded_h5ad is not None:
    ...
    with st.expander("📚 Parameter Guide", expanded=False):
        ...
        - **Change cluster order**: Change cluster display order via drag **Change cluster order**
        ...
        - **Visualizations requiring Vector Field** (Streamline, Cell-wise vectors, Grid vectors, Topography):
          - Only bases with calculated Vector Field can be selected
          - If Vector Field is calculated for multiple bases in Dynamo Analysis, all are shown as options
        - **Geometric features panel**:
          - If Geometry is calculated for multiple bases, **panels are automatically generated for all bases**
          - Independent panels are displayed for each basis (for comparison)
        - **Other visualizations** (Basic UMAP):
          - Can select from all embeddings in the data
        - Common embeddings: umap, mnn_umap, harmony_umap, pca, tsne, draw_graph_fa, etc.
        - UMAP recommended (if available)
        - ⚠️ **UMAP required for Topography analysis**: Topography is difficult in high-dimensional space,
          only works accurately with UMAP-based vector field
        ...
        - **quiver_size**: Vector width (cell-wise/grid vectors)
        """)
        ...
        if viz_type in ["Streamline plot", "Cell-wise vectors", "Grid vectors", "Topography (potential landscape)", "Basic UMAP"]:
            ...
            if color_type == "Categorical" and categorical_cols:
                ...
                if color_col:
                    ...
                    with st.sidebar:
                        ...
                        show_cluster_labels = st.checkbox(
                            ...
                            value=True,
                            help="Show labels on each cluster (when off, only legend is shown)"
                        )
                ...
                topo_feature = st.radio(
                    ...
                    index=0,
                    help="Calculate and visualize potential and pseudotime using Fokker-Planck method"
                )
                ...
                if not has_potential_fp or not has_pseudotime_fp:
                    st.info(f"✓ {topo_feature} will be calculated when "Generate Visualization" is clicked")
                else:
                    st.success(f"✓ {topo_feature} has already been calculated")
    ...
    with col2:
        ...
        if viz_type in ["Streamline plot", "Cell-wise vectors", "Grid vectors", "Topography (potential landscape)"]:
            if has_vector_field:
                ...
                if viz_type == "Topography (potential landscape)":
                    ...
                    if not available_bases_for_viz:
                        st.error("❌ No **2D embedding** with calculated Vector Field found.")
                        st.warning(f"""
                        **Requirements for Topography analysis:**
                        - ✅ 2D embedding (UMAP, tSNE, etc.)
                        - ✅ Vector Field must be calculated in that 2D space

                        **Current status:**
                        - Available Vector Fields: {', '.join(vecfld_bases) if vecfld_bases else 'None'}

                        **Solution:**
                        1. Calculate Vector Field using **2D embedding like UMAP** in Dynamo Analysis
                        ...
                        **Note:** Topography does not work with high-dimensional embeddings (PCA, etc.).
                        Vector Field is also calculated in high-dimensional space, so it cannot be reduced to 2D.
                        """)
                        basis = None
                    else:
                    ...
                    basis = st.selectbox(
                        ...
                        available_bases_for_viz,
                        index=default_basis_idx,
                        help="Embedding used for visualization. If Vector Field exists, any embedding can be used.\n\n"
                             f"Vector Field calculated: {', '.join(vecfld_bases)}"
                    )

                    st.caption(f"ℹ️ Vector Field: {', '.join(vecfld_bases)}")
            else:
                st.error("❌ Vector Field not found. Please calculate in Dynamo Analysis.")
                basis = None
            ...
            if geometry_by_basis:
                st.info(f"📊 **Auto panel generation mode**\n\nAutomatically generating panels for all calculated bases ({len(geometry_by_basis)} total):\n- {', '.join(geometry_by_basis.keys())}")
            else:
                st.warning("⚠️ No bases with calculated Geometry found")
        else:
            ...
            basis = st.selectbox(
                ...
                index=default_basis_idx,
                help="Embedding used for visualization (only showing those in data)"
            )
    ...
    if st.button("🎨 Generate Visualization", type="primary"):
        ...
        try:
            ...
            if viz_type == "Streamline plot":
                ...
                with st.spinner("Generating streamline plot..."):
                    ...
                    with col_dl2:
                        try:
                            ...
                            st.warning(f"⚠️ PDF save failed (contains non-finite numbers): {str(e)}\n\nPlease use PNG version.")
                        except Exception as e:
                            st.warning(f"⚠️ PDF save failed: {str(e)}\n\nPlease use PNG version.")
                ...
                if basis is None:
                    st.warning("⚠️ No 2D embedding with calculated Vector Field found. Please calculate in Dynamo Analysis.")
                else:
                    ...
                    if current_color_type == "Potential/Pseudotime (FP)" and selected_topo_feature and selected_topo_feature != "None":
                        ...
                        if has_potential_fp and has_pseudotime_fp:
                            st.success(f"✅ {selected_topo_feature} has already been calculated. Using existing values.")
                        ...
                        # Compute if not already computed
                            ...
                            with st.spinner(f"Computing {selected_topo_feature} using Fokker-Planck method..."):
                                try:
                                    ...
                                    st.caption("⚙️ Computing cell velocities for transition matrix...")
                                    ...
                                    st.caption("⚙️ Computing potential using Hodge decomposition...")
                    ...
                    with st.spinner("Generating topography plot..."):
                        ...
                        while not topography_success and retry_count < max_retries:
                            try:
                                ...
                                if not try_frontier:
                                    st.info("ℹ️ Skipped fixed points analysis")
                            except (TypeError, AttributeError) as e:
                                retry_count += 1
                                if try_frontier and "NoneType" in str(e) and retry_count < max_retries:
                                    # Retry without frontier (fixed points)
                                    st.warning("⚠️ Error in fixed points analysis. Retrying with frontier=False...")
                                    try_frontier = False
                                    plt.close('all')
                                    fig, ax = plt.subplots(figsize=(fig_width, fig_height))
                                else:
                                    # If frontier=False also fails
                                    st.error(f"""
                                    ...
                                    **Possible causes:**
                                    - There may be an issue with Vector Field calculation
                                    - Data dimensions may be inappropriate

                                    **Solution:**
                                    1. Please try other visualization types (Streamline plot, etc.)
                                    2. Please recalculate Vector Field in Dynamo Analysis
                                    """)
                        ...
                        if topography_success:
                            ...
                            with col_dl1:
                                if 'topography_png' in st.session_state:
                                    ...
                                    st.info("💡 The figure is displayed in the browser. Please use screenshot.")
                            ...
                            with col_dl2:
                                if st.session_state.get('topography_pdf') is not None:
                                    ...
                                    st.warning(f"⚠️ PDF save failed: {st.session_state['topography_pdf_error']}\n\nPlease use PNG version.")
                                else:
                                    st.warning("⚠️ PDF save failed\n\nPlease use PNG version.")
    ...
    st.info("👆 Upload h5ad file generated by Dynamo analysis app to start")
