...
### Visualization Types
- **Streamline plot**: Represents vector field flow as smooth curves
- **Cell-wise vectors**: Displays vectors as arrows for each cell
...
- **Geometric features panel**: Visualization of speed, divergence, acceleration, curvature
  - **Multi-basis support**: When geometry is computed for multiple bases, panels are automatically generated for all bases

### Multi-basis Analysis Support
...
if uploaded_h5ad is not None:
    ...
    with st.expander("📚 Parameter Guide", expanded=False):
        ...
        - **Change cluster order**: Change display order of clusters by drag & drop
        ...
        - **Visualizations requiring Vector Field** (Streamline, Cell-wise vectors, Grid vectors, Topography):
          ...
          - If Vector Field was computed for multiple bases in Dynamo Analysis, all are shown as options
        - **Geometric features panel**:
          - When Geometry is computed for multiple bases, **panels are automatically generated for all bases**
        ...
        - Warning: **UMAP is required for Topography analysis**: Since Topography is difficult to analyze in high-dimensional space,
          it only works accurately with UMAP-based vector fields
        ...
        if viz_type in ["Streamline plot", "Cell-wise vectors", "Grid vectors", "Topography (potential landscape)", "Basic UMAP"]:
            ...
            if color_type == "Categorical" and categorical_cols:
                ...
                if color_col:
                    ...
                    with st.sidebar:
                        ...
                        show_cluster_labels = st.checkbox(
                            ...
                            value=True,
                            help="Display labels on each cluster (when off, only the legend is shown)"
                        )
                ...
                topo_feature = st.radio(
                    ...
                    index=0,
                    help="Compute potential and pseudotime using Fokker-Planck method for visualization"
                )
                ...
                if not has_potential_fp or not has_pseudotime_fp:
                    st.info(f"✓ {topo_feature} will be computed when you click 'Generate Visualization'")
                else:
    ...
    with col2:
        ...
        if viz_type in ["Streamline plot", "Cell-wise vectors", "Grid vectors", "Topography (potential landscape)"]:
            if has_vector_field:
                ...
                if viz_type == "Topography (potential landscape)":
                    ...
                    if not available_bases_for_viz:
                        st.error("No **2D embedding** with a computed Vector Field was found.")
                        st.warning(f"""
                        **Topography analysis requirements:**
                        - A 2D embedding (UMAP, tSNE, etc.)
                        - Vector Field must be computed in that 2D space

                        **Current state:**
                        - Available Vector Fields: {', '.join(vecfld_bases) if vecfld_bases else 'None'}

                        **Solution:**
                        1. Compute Vector Field using a **2D embedding such as UMAP** in Dynamo Analysis
                        2. Or use another visualization type (e.g., Streamline plot)

                        **Note:** Topography does not work with high-dimensional embeddings (e.g., PCA).
                        The Vector Field is also computed in high-dimensional space and cannot be reduced to 2D.
                        """)
                        basis = None
                    else:
                        ...
                        basis = st.selectbox(
                            ...
                            help="For Topography analysis: requires 2D embedding + Vector Field"
                        )
                    ...
                    basis = st.selectbox(
                        ...
                        available_bases_for_viz,
                        index=default_basis_idx,
                        help="Embedding used for visualization. Any embedding can be used if a Vector Field exists.\n\n"
                             f"Vector Field computed for: {', '.join(vecfld_bases)}"
                    )
            ...
            if geometry_by_basis:
                ...
                st.warning("No basis with computed Geometry was found")
        else:
            ...
            basis = st.selectbox(
                ...
                index=default_basis_idx,
                help="Embedding used for visualization (only those present in the data are shown)"
            )
    ...
    if st.button("🎨 Generate Visualization", type="primary"):
        ...
        try:
            ...
            if viz_type == "Streamline plot":
                ...
                if basis is None:
                    st.warning("No 2D embedding with a computed Vector Field was found. Please compute it in Dynamo Analysis.")
                else:
                        ...
                        while not topography_success and retry_count < max_retries:
                            try:
                                ...
                                if try_frontier and "NoneType" in str(e) and retry_count < max_retries:
                                    ...
                                    st.warning("Error occurred during fixed points analysis. Retrying with frontier=False...")
                                    try_frontier = False
                                    plt.close('all')
                                    fig, ax = plt.subplots(figsize=(fig_width, fig_height))
                                else:
                                    # Also failed with frontier=False
                                    st.error(f"""
                                    ...
                                    **Possible causes:**
                                    - There may be an issue with the Vector Field computation
                                    - The data dimensions may be inappropriate

                                    **Solutions:**
                                    1. Try other visualization types (e.g., Streamline plot)
                                    2. Recompute the Vector Field in Dynamo Analysis
                                    """)
                        ...
                        if topography_success:
                            ...
                            with col_dl1:
                                if 'topography_png' in st.session_state:
                                    ...
                                    st.info("The figure is displayed in the browser. Please use a screenshot instead.")
    ...
    st.info("Upload an h5ad file generated by the Dynamo Analysis app to get started")
