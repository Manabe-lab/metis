...
def create_cell_color_mapping(cell_list, palette_name):
    """
    Function to create consistent mapping between cluster/cell type and color
    ...
    # Extend if palette does not have enough colors
...
def add_cluster_annotation_bar(ax, pseudotime_sorted, cluster_sorted, bins, cluster_color_dict, reverse_pseudotime=False, height_inches=0.15):
    """
    Function to add cluster annotation bar below the plot (under x-axis label)

    Use axes_divider to place annotation bar below x-axis
    ...
    reverse_pseudotime : bool
        Whether to invert pseudotime direction
    height_inches : float
        Height of annotation bar (in inches)
    """
...
### Visualization Types
1. **Gene expression - Line plot**: Display gene expression changes along pseudotime as line graph
   - **Individual genes**: Separate plots for each gene
   - **Overlay genes**: Overlay all genes in one graph
2. **Gene expression - Heatmap**: Display expression patterns of multiple genes as heatmap
3. **Cluster density**: Visualize cluster composition changes along pseudotime
   - **Line plot**: Density change for each cluster
   - **Stacked area**: Cluster composition transition (100% stacked)
   - **Heatmap**: 2D display of cluster density

### Available Pseudotime
- **Velocity pseudotime** (scVelo) - Pseudotime calculated from RNA velocity analysis
- **Latent time** (scVelo dynamical model) - Latent time based on dynamic model
- **DPT pseudotime** (Diffusion pseudotime) - Diffusion pseudotime
- **Fate probabilities** (CellRank) - Fate probability to each terminal state (`fate_prob_{state}` columns)
  - X-axis = 0 (low probability) to 1 (high probability)
  - Visualize gene expression changes during differentiation to specific cell fate
...
uploaded_h5ad = st.file_uploader(
    ...
    help="h5ad file from scVelo/CellRank analysis"
)
...
if uploaded_h5ad is not None:
    ...
    reverse_pseudotime = st.sidebar.checkbox(
        ...
        value=False,
        help="Reverse pseudotime direction (endpoint → starting point)"
    )

    log1p_pseudotime = st.sidebar.checkbox(
        "Log1p transform pseudotime (x-axis)",
        value=False,
        help="Display pseudotime as log1p transformed x-axis (log(1+x)). Useful when pseudotime distribution is skewed"
    )
    ...
    if analysis_type == "Gene expression":
        ...
        if gene_input_method == "Multi-select":
            selected_genes = st.multiselect(
                ...
                gene_list,
                max_selections=50,
                help="Select genes to display (max 50)"
            )
        else:
            gene_text_input = st.text_area(
                "Enter gene names",
                height=150,
                help="Enter gene names separated by space, comma, tab, or newline"
            )
    ...
    if analysis_type == "Gene expression" and viz_type == "Line plot":
        ...
        line_plot_mode = st.radio(
            ...
            help="Individual: Separate plots for each gene\nOverlay: Overlay all genes in one graph"
        )
        ...
        with col1:
            n_bins = st.slider(
                "Number of bins",
                5, 100, 10, 5,
                help="Number of bins for pseudotime (fewer bins gives smoother results)"
            )
        with col2:
            show_scatter = st.checkbox(
                "Show scatter points",
                value=False,
                help="Display individual cells as scatter plot"
            )
        with col3:
        ...
        with col5:
            use_zscore_line = st.checkbox(
                ...
                value=False,
                help="Z-score normalize each gene (mean 0, std 1)"
            )
        with col6:
            line_color = st.selectbox(
                ...
                index=0,
                help="Line color (for Individual genes)"
            )
        ...
        split_by_cluster = st.checkbox(
            "Split by cluster",
            value=False,
            help="Display gene expression trends separated by cluster"
        )
            ...
            if not categorical_columns:
                ...
                with col_palette:
                    split_cluster_palette = st.selectbox(
                        ...
                        index=0,
                        help="Color palette for clusters"
                    )
        ...
        show_cluster_annotation_line = st.checkbox(
            ...
            value=False,
            help="Display dominant cluster for each pseudotime range as color bar below graph",
        ...
        with col1:
            n_bins = st.slider(
                "Number of bins",
                10, 200, 100, 10,
                help="Number of bins for pseudotime"
            )
        with col2:
        ...
        if use_clustermap:
            ...
            with col4:
                cluster_rows = st.checkbox(
                    "Cluster genes (rows)",
                    value=True,
                    help="Hierarchically cluster genes (rows)"
                )
            with col5:
            ...
            cluster_genes = st.checkbox(
                "Cluster genes",
                value=True,
                help="Hierarchically cluster genes by expression pattern (no dendrogram)"
            )
        ...
        show_cluster_annotation = st.checkbox(
            ...
            value=False,
            help="Display dominant cluster for each bin above heatmap"
        )
        ...
        normalize_cluster_size = st.checkbox(
            ...
            value=False,
            help="Compare with equal total cells per cluster. Removes cluster size effects and visualizes relative density changes."
        )
            ...
            if show_cluster_annotation_density:
                ...
                with col_ann_cluster:
                    annotation_cluster_key_density = st.selectbox(
                        "Cluster identity for annotation",
                        categorical_columns,
                        help="Select cluster column for annotation(Can select different column from the one used in Cluster density)",
    ...
    st.info("👆 Upload h5ad file to start")
