...
st.markdown("""
Using scCODA, statistically detect **changes in cell type composition** in single-cell data.

### Features of scCODA
- **Handles Compositional Data**: Properly processes the compositional constraint of proportion data
- **Hierarchical Bayesian Model**: Estimates cell type proportion changes between groups
- **Sparse Estimation**: Detects only cell types that are actually changing
- **Reference Cell Type**: Allows selection of a cell type assumed to be unchanged
- **Bayesian Statistics**: Determines significance using Inclusion probability instead of p-values or FDR

### Workflow
1. **Upload h5ad file**: Data containing cell type, sample, and condition information
2. **Parameter Settings**: Select cell type column, sample column, condition column, and reference cell type
3. **Run scCODA**: Differential composition analysis using Bayesian estimation
...
The h5ad file (`adata`) must contain the following information:

- **Cell type information**: Included as a column in `adata.obs` (e.g., `'cell_type'`, `'clusters'`, `'leiden'`)
- **Sample information**: Included as a column in `adata.obs` (e.g., `'sample'`, `'batch'`, `'replicate'`)
- **Condition information**: Included as a column in `adata.obs` (e.g., `'condition'`, `'group'`, `'treatment'`)

Tip: If you analyzed data with Seurat, please convert it to `anndata` format in Python before uploading.
""")
...
if uploaded_h5ad is not None:
    ...
    if "sccoda_file_hash" not in st.session_state or st.session_state.sccoda_file_hash != file_hash:
        with st.spinner("Loading h5ad file..."):
            try:
                ...
                st.success(f"h5ad file loaded: {adata.shape[0]} cells x {adata.shape[1]} genes")
        ...
        st.success(f"h5ad file (cached): {adata.shape[0]} cells x {adata.shape[1]} genes")
    ...
    with col2:
        st.metric("Number of Genes", f"{adata.shape[1]:,}")
    with col3:
...
if st.session_state.sccoda_adata is not None:
    ...
    # Filtering: Only columns with less than 50 unique values are candidates
    filtered_columns = [col for col in obs_columns if adata.obs[col].nunique() < 50]

    if len(filtered_columns) == 0:
        st.error("No columns with fewer than 50 unique values found. Please check the metadata columns.")
        st.stop()

    # Function to select default columns
    def find_default_celltype_col(columns, obs_df):
        """Find default for cell type column"""
        ...
        if type_cols:
            return type_cols[0]
        # 3. Default is the first column
        return columns[0]

    def find_default_sample_col(columns, obs_df):
        """Find default for sample column"""
        ...
        # 4. Default is the first column
        return columns[0]

    def find_default_condition_col(columns, obs_df):
        """Find default for condition column"""
        # Search for columns containing condition, stim, KO
        keywords = ['condition', 'stim', 'ko']
        for keyword in keywords:
            matching_cols = [col for col in columns if keyword in col.lower()]
            if matching_cols:
                return matching_cols[0]
        # Default is the second column (the first is likely to be used as sample)
    ...
    current_reference = st.session_state.sccoda_params.get("reference_celltype", "Automatic")
    ...
    st.info("After selecting parameters, click the 'Confirm Parameters' button")
    with st.form("sccoda_params_form"):
        ...
        with col2:
            ...
            ref_options = ["Automatic"] + all_celltypes
            ...
            reference_celltype = st.selectbox(
                ...
                ref_options,
                index=ref_idx,
                help="Cell type assumed to be unchanged. If Automatic, the most stable cell type will be selected."
            )
            ...
            inclusion_threshold = st.slider(
                ...
                step=0.05,
                help="Threshold for determining significance. 0.95 means the effect is non-zero in 95% of MCMC samples"
            )
    ...
    reference_celltype = st.session_state.sccoda_params.get("reference_celltype", "Automatic")
    ...
    formula = st.text_input(
        ...
        value=condition_col,
        help="Default is condition column only. To include additional covariates, specify like 'condition + batch'"
    )
    ...
    with st.expander("Summary of Selected Data (confirmed)"):
        ...
        **Cell Type Column:** `{celltype_col}`
        - Number of unique cell types: {adata.obs[celltype_col].nunique()}
        ...
        **Condition Column:** `{condition_col}`
        ...
        # Cross-tabulation
        st.markdown("**Sample x Condition Cross-tabulation:**")
    ...
    if st.button("Set Conditions and Check Data Distribution", type="secondary"):
        with st.spinner("Visualizing data distribution..."):
            try:
                ...
                st.markdown("#### Cell Type Composition (by condition, cell-level aggregation)")
                st.caption("* Proportions calculated by counting all cells (same method as original barplot)")
                try:
                    import matplotlib.pyplot as plt
                ...
                with st.expander("Show Sample-level Average (scCODA method)"):
                    st.caption("* Proportions calculated per sample, then averaged within condition (method used for statistical analysis)")
                    try:
                        import matplotlib.pyplot as plt
                        fig = sccoda_vis.plot_stacked_barplot(
                            adata_vis,
                        ...
                        st.pyplot(fig)
                        # Save figure for PDF
                        st.session_state.sccoda_plots["step2"].append(("Cell Type Composition (by condition, sample average)", fig))
                    except Exception as e:
    ...
    if st.button("Run scCODA Analysis", type="primary"):
        with st.spinner("Running scCODA..."):
            try:
                ...
                st.success(f"scCODA data created: {adata_sccoda.shape}")
                ...
                if reference_celltype != "Automatic":
                    st.info(f"Reference cell type: {reference_celltype}")
                    sccoda.prepare(
                        adata_sccoda,
                        formula=formula,
                    ...
                    st.info("Automatically selecting reference cell type...")
                    sccoda.prepare(
                        adata_sccoda,
                        formula=formula,
                ...
                # Run scCODA
                st.info("Running Bayesian estimation (this may take a few minutes)...")
                sccoda.run_nuts(
                    adata_sccoda,
                    num_warmup=1000,
                    num_samples=1000
    ...
    if st.session_state.sccoda_effect_df is not None:
        ...
        **Interpreting scCODA results:**
        - **Final Parameter**: Posterior mean estimate of the effect (0 = not significant, non-zero = significant)
        - **Inclusion probability**: Probability that the effect is non-zero (higher = more certain change)
        - **log2-fold change**: Log2-transformed fold change
        - **HDI**: High-density interval (Bayesian version of confidence interval)
        """)
        ...
        if 'Final Parameter' in effect_df.columns:
            ...
            if 'Inclusion probability' in effect_df.columns:
                ...
                if len(significant_high_prob) > 0:
                    st.success(f"**{len(significant_high_prob)} cell type(s) with significant changes detected** (Inclusion prob >= {thresh})")
                    st.markdown("#### Cell Types with Significant Changes")
                    st.dataframe(significant_high_prob, use_container_width=True)
                else:
                    st.warning(f"No cell types met the threshold of Inclusion probability >= {thresh}")
                    if len(significant) > 0:
                        st.info(f"However, changes were detected in {len(significant)} cell type(s) (below threshold)")
            else:
                if len(significant) > 0:
                    st.success(f"**{len(significant)} cell type(s) with significant changes detected**")
                    st.dataframe(significant, use_container_width=True)
                else:
                    st.info("No cell types with significant changes were detected")
        ...
        with col1:
            ...
            if 'Final Parameter' in effect_df.columns and (effect_df['Final Parameter'] != 0).any():
                ...
                st.info("No effect plot displayed as there are no significant changes")
...
# ========================================
with st.expander("About scCODA"):
    st.markdown("""
    ## What is scCODA?

    scCODA (single-cell Compositional Data Analysis) is a hierarchical Bayesian model for statistically detecting
    **changes in cell type composition** in single-cell data.

    ### Key Features

    1. **Handles Compositional Data**
       - Cell type proportions have a compositional constraint (sum = 1)
       - Standard statistical methods are inappropriate
       - scCODA is specialized for compositional data

    2. **Sparse Estimation**
       - Detects only cell types that are actually changing
       - Reduces false positives

    3. **Hierarchical Model**
       - Accounts for variation between samples
       - Distinguishes biological variation from technical variation

    4. **Reference Cell Type**
       - Select a cell type assumed to be unchanged
       - Evaluate changes in other cell types relative to this reference

    5. **Bayesian Statistical Inference**
       - Uses **Inclusion probability** instead of p-values or FDR
       - Inclusion probability is the proportion of MCMC samples where the effect is non-zero
       - Typically 0.95 or higher is considered significant (95% probability of change)

    ### When to Use?

    - **Comparing cell type proportions between 2 or more conditions**
    - **Multiple biological replicates are available**
    - **Statistical analysis that accounts for compositional data properties is needed**

    ### Comparison with Other Methods

    | Method | Data | Between-sample variation | Compositional constraint | Sparse estimation | Statistical method |
    |--------|------|--------------------------|--------------------------|-------------------|-------------------|
    | **scCODA** | Count | Yes | Yes | Yes | Bayesian (Inclusion prob) |
    | **Propeller** | Count | Yes | No | No | Frequentist (p-value) |
    | **Beta-binomial** | Count | Yes | No | No | Frequentist (p-value/LRT) |
    | **Chi-squared test** | Count | No | No | No | Frequentist (p-value) |

    ### References

    - Buttner, M., Ostner, J., Muller, C.L. et al. (2021)
