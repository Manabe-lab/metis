...
st.markdown("""
This app filters loom files generated by velocyto based on cells in h5ad files,
and can merge multiple loom files.

### Workflow
1. **Load h5ad file**: AnnData format file containing cell metadata
2. **Select identity**: Select metadata column with sample/group info corresponding to loom files
3. **Upload loom files**: Loom files generated by velocyto (multiple allowed)
4. **Group assignment**: Select which group each loom file corresponds to
5. **Filter & merge**: Filter cells based on barcode and merge multiple files
6. **Download**: Download filtered loom file
""")
...
if uploaded_h5ad is not None:
    ...
    if use_all_cells:
        st.info("ℹ️ All h5ad cells will be matched. No group splitting will be performed.")
        ...
        with st.form("identity_form"):
            st.markdown("""
            Please select a metadata column containing sample or group information corresponding to loom files.
    ...
    if use_all_cells or (not use_all_cells and "identity_col" in st.session_state and st.session_state.identity_col is not None):
        ...
        if uploaded_looms:
            ...
            if use_all_cells:
                ...
                st.info("ℹ️ Skipping group assignment due to all-cells mode. All loom files will be matched against the same h5ad cell set.")
                ...
                with st.form("loom_assignment_form"):
                    st.markdown("""
                    Please select which group each loom file corresponds to.
                    """)
            ...
            if "loom_to_group" in st.session_state:
                ...
                if st.session_state.filtered_loom_ready:
                    ...
                    Using this filtered loom file and h5ad file,
                    you can run RNA velocity analysis (e.g., scVelo).
    ...
    st.info("👆 Please upload an h5ad file to start")
