...
st.markdown("""
This app allows you to filter loom files generated by velocyto based on cells in h5ad files,
and merge multiple loom files.

### Workflow
1. **Load h5ad file**: Anndata format file containing cell metadata
2. **Select Identity**: Choose the metadata column containing sample/group information corresponding to loom files
3. **Upload Loom files**: Upload loom file(s) generated by velocyto (multiple files supported)
4. **Assign groups**: Select which group each loom file corresponds to
5. **Filter & Merge**: Filter cells based on barcodes and merge multiple files
6. **Download**: Download the filtered loom file
""")
...
if uploaded_h5ad is not None:
    ...
    use_all_cells = st.checkbox(
        ...
        value=False,
        help="Match all cells at once. If there are multiple loom files, all will be matched against the same h5ad cell set."
    )

    if use_all_cells:
        st.info("ℹ️ All h5ad cells will be used for matching. No group splitting will be performed.")
        ...
        with st.form("identity_form"):
            st.markdown("""
            Select the metadata column containing sample or group information corresponding to loom files.
            Examples: `sample`, `patient`, `condition`, etc.
            """)
            ...
            identity_col = st.selectbox(
                ...
                metadata_cols,
                index=default_index,
                help="Column containing group information corresponding to each loom file"
            )
    ...
    if use_all_cells or (not use_all_cells and "identity_col" in st.session_state and st.session_state.identity_col is not None):
        ...
        uploaded_looms = st.file_uploader(
            ...
            key="loom_upload",
            help="Upload one or more loom files generated by velocyto"
        )
        ...
        if uploaded_looms:
            ...
            if use_all_cells:
                ...
                st.info("ℹ️ Skipping group assignment because all cells mode is enabled. All loom files will be matched against the same h5ad cell set.")
                ...
                with st.form("loom_assignment_form"):
                    st.markdown("""
                    Select which group each loom file corresponds to.
                    """)
            ...
            if "loom_to_group" in st.session_state:
                ...
                if st.session_state.filtered_loom_ready:
                    ...
                    You can use this filtered loom file along with the h5ad file to
                    perform RNA velocity analysis (e.g., scVelo).
    ...
    st.info("👆 Upload an h5ad file to get started")
