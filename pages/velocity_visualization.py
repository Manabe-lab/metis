...
4. **Velocity confidence**: Velocity reliability
5. **Velocity pseudotime**: Velocity-based pseudotime
6. **Gene-specific velocity**: Velocity for specific genes
7. **PAGA**: Partition-based graph abstraction (inter-cluster connectivity)

**Note:** Gene expression changes along pseudotime can be visualized in the **Pseudotime gene expression** app.
...
uploaded_h5ad = st.file_uploader(
    ...
    help="h5ad file generated by the scVelo analysis app"
)
...
if uploaded_h5ad is not None:
    ...
    if required_data:
        ...
        This file does not appear to be an scVelo analysis result.
        ...
        if len(velocity_sources) > 1:
            ...
            with col_vel1:
                selected_velocity = st.radio(
                    ...
                    help="Switch between different velocity calculation results to compare"
                )
            with col_vel2:
    ...
    with st.expander("📚 Visualization Guide", expanded=False):
        st.markdown("""
        ### Embedding Basis
        - **Multiple selection available**: You can select UMAP, tSNE, PCA, etc. simultaneously
        ...
        - **Continuous colormap**: For continuous variables (gene expression, pseudotime, etc.)
          - Automatically shown/hidden based on visualization type
        - **Show cluster names**: Toggle legend display on/off

        ### Cluster Settings (Sidebar)
        - **Change cluster order**: Change cluster display order via drag & drop
        - Order changes are also reflected in color assignments

        ### Velocity Plots
        - **Stream plot**: Continuous velocity flow
          - Understand overall flow and trajectories
          - Advanced options: density, smoothing, min_mass
        - **Grid plot**: Grid-based velocity arrows
          - Detailed view of individual cell velocity directions
        ...
        - **Min mass**: Minimum mass threshold for displaying arrows
        - **Scale**: Arrow scale (0.1-2.0)
        - **Alpha**: Arrow transparency (0.1-1.0)

        ### PAGA (Partition-based graph abstraction)
        - Visualize inter-cluster connectivity
        - Velocity-directed PAGA shows directional transitions between clusters
        - **PAGA graph**: Inter-cluster connectivity graph
        - **PAGA compare**: Side-by-side comparison of PAGA graph and UMAP/tSNE
        - Not displayed if PAGA has not been computed

        ### Download
        - **PNG format**: High-resolution bitmap (300 DPI)
        - **PDF format**: Vector format (ideal for publication and printing)
        - All generated plots are downloadable in both formats
        """)
    ...
    with st.sidebar:
        ...
        show_legend = st.checkbox(
            ...
            value=True,
            help="Whether to display the legend with cluster/category names"
        )
    ...
    with col1:
        ...
        basis_selections = st.multiselect(
            "Embedding basis (embedding space for visualization)",
            available_bases,
            default=[default_basis] if default_basis else [],
            help="Select the embedding space for visualization. Multiple selections will generate plots for each embedding"
        )
    ...
    if viz_type in ["Velocity embedding stream", "Velocity embedding grid", "Gene velocity"]:
        split_by_enabled = st.checkbox(
            ...
            value=False,
            help="Create individual plots for each category in the metadata"
        )
        ...
        if split_by_enabled:
            split_by_column = st.selectbox(
                ...
                categorical_columns,
                help="Split plots by each category in this column"
            )
        else:
    ...
    if viz_type == "Velocity embedding stream":
        ...
        with st.expander("Advanced stream options"):
            ...
            with col2:
                stream_smooth = st.slider("Smoothing", 0.0, 1.0, 0.5, 0.1,
                                         help="Smoothness of streamlines")
            with col3:
        ...
        gene_selection_method = st.radio(
            ...
            help="Top velocity genes: Automatically select genes with the highest velocity"
        )
        ...
        selected_genes = st.multiselect(
            ...
            gene_list,
            max_selections=10,
            help="Select genes to display (maximum 10)"
        )
        ...
        paga_plot_type = st.radio(
            ...
            help="PAGA graph: Inter-cluster connectivity graph only\nPAGA compare: Side-by-side comparison of PAGA graph and UMAP/tSNE"
        )
        ...
        with col1:
            paga_threshold = st.slider(
                "Edge threshold",
                0.0, 1.0, 0.01, 0.01,
                help="Threshold for displaying edges (lower values show more connections)"
            )
        with col2:
            paga_node_size_scale = st.slider(
                "Node size scale",
                0.5, 5.0, 1.0, 0.5,
                help="Scale for node sizes"
            )
        with col3:
            paga_arrowsize = st.slider(
                "Arrow size",
                0, 30, 15, 1,
                help="Size of directional arrows (0 for no arrows)"
            )
    ...
    st.info("👆 Please upload an scVelo analysis result h5ad file to begin")
