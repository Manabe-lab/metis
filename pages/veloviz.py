...
st.markdown("""
Create 2D embeddings that incorporate RNA velocity. VeloViz uses both the current expression state and the predicted future expression state
to construct an embedding space for visualizing cell trajectories.

### Features
- Unlike conventional UMAP/tSNE, velocity information is reflected in the embedding
- Cell differentiation directions are visualized more clearly
- Can be computed directly from scVelo analysis results
...
if not veloviz_available:
    ...
    Also, rpy2 is required for Python:
    ```bash
    pip install rpy2
    ```
    """)
...
uploaded_h5ad = st.file_uploader(
    ...
    help="h5ad file analyzed with scVelo (velocity layer required)"
)
    ...
    with st.expander("📊 Parameter settings", expanded=True):
        ...
        with col1:
            n_pcs = st.slider(
                ...
                help="Number of principal components. Higher values provide more detail but increase computation cost"
            )

            k_neighbors = st.slider(
                "Number of neighbors (k):",
                min_value=5, max_value=100, value=30, step=5,
                help="Number of neighbors for each cell. Larger values result in smoother embeddings"
            )

            similarity_threshold = st.slider(
                "Similarity threshold:",
                min_value=0.0, max_value=1.0, value=0.2, step=0.05,
                help="Similarity threshold with velocity direction. Higher values are more stringent"
            )
        ...
        with col2:
            distance_weight = st.slider(
                ...
                help="Weight for the distance component"
            )

            normalize_depth = st.checkbox(
                "Normalize depth",
                value=True,
                help="Whether to normalize by depth"
            )

            use_ods_genes = st.checkbox(
                "Use overdispersed genes",
                value=True,
                help="Whether to use overdispersed genes"
            )
    ...
    if st.button("🚀 Run VeloViz", type="primary", key="run_veloviz"):
        with st.spinner("Running VeloViz... This may take a few minutes."):
            try:
                ...
                **Troubleshooting:**
                1. Check if the data contains many NaN values
                2. Check if the number of cells is too small (recommended: >500 cells)
                3. Verify that velocity calculation was performed correctly
                """)
                import traceback
